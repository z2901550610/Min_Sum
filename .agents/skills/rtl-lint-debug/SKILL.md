...
After `source scripts/eda-env.sh`, reproduce the aggregate failure with
`make lint`. Once the failing layer is known, use the smallest owning Make target
for iteration and read the quiet wrapper log under `build/logs/verilator/`; use
...
target and finish with `make lint`. Lint success is not simulation, synthesis,
or timing evidence.
