...
VERILATOR_FLAGS ?= --binary --sv -DBIKE_TOY_PARAMS -DBIKE_PARALLEL_L=$(BIKE_PARALLEL_L) $(RAM_LANE_DEPTH_DEFINE) -DBIKE_RAM_M_ROW_BANKS=$(RAM_M_ROW_BANKS) -DBIKE_SIM_DEBUG -Wall -Wno-fatal -I./tb -I./rtl
...
BIKE_RANDOM_RAM_LANE_MIN_DEPTH ?= 3
...
$(RAM_I_HEX_STAMP): FORCE rtl/bike_pkg.sv scripts/gen_qc_first_columns.py scripts/ram_i_hex.py scripts/qc_matrix_data.py scripts/select_row_bank_count.py
	@python3 scripts/gen_qc_first_columns.py --input rtl/bike_pkg.sv --output-dir $(RAM_I_HEX_DIR)/ --parallel-l $(BIKE_PARALLEL_L) $(RAM_LANE_DEPTH_ARG)
	@touch $@

$(RAM_I_HEX): $(RAM_I_HEX_STAMP)
	@if [ ! -f "$@" ]; then python3 scripts/gen_qc_first_columns.py --input rtl/bike_pkg.sv --output-dir $(RAM_I_HEX_DIR)/ --parallel-l $(BIKE_PARALLEL_L) $(RAM_LANE_DEPTH_ARG); touch $(RAM_I_HEX_STAMP); fi
...
test-unit: $(VECTOR_SVH) $(RAM_I_HEX_STAMP)
	...
	@$(VERILATOR) $(VERILATOR_FLAGS) --top-module tb_ram_blocks rtl/bike_pkg.sv rtl/ram_1r1w_sync_read.sv rtl/ram_c.sv rtl/ram_m.sv rtl/ram_s.sv rtl/ram_syndrome.sv rtl/ram_t.sv tb/tb_ram_blocks.sv
	...
	@$(VERILATOR) $(VERILATOR_FLAGS) --top-module tb_edge_message_pipe rtl/bike_pkg.sv rtl/edge_message_pipe.sv tb/tb_edge_message_pipe.sv
...
test-bike-random:
	@python3 scripts/run_bike_random.py --base-seed $(BIKE_RANDOM_BASE_SEED) --trials $(BIKE_RANDOM_TRIALS) --error-count $(BIKE_RANDOM_ERROR_COUNT) --parallel-l $(BIKE_RANDOM_PARALLEL_L) --ram-lane-min-depth $(BIKE_RANDOM_RAM_LANE_MIN_DEPTH) --verilator $(VERILATOR)
