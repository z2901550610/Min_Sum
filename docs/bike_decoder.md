...
本 RTL 实现了一个 BIKE/MDPC 风格的 syndrome 输入 min-sum 解码器。校验矩阵为双循环矩阵：
...
其中 `W` 是每个 circulant block 的列权重，BIKE-L1 的总行重为 `N0 * W = 142`。RAM-I、RAM-S 和 RAM-T 的 lane 内 entry 深度统一为 `RAM_LANE_DEPTH`。
...
RAM-I 保存 `{one_idx, row_idx_global}` entry。解码期间，c2v 侧按 `h_block_idx` 和 `entry_pos` 读取首列 entry，`decoder_top` 对 `base_row_idx_global + col_idx_local` 执行环绕修正，得到活跃列的校验行地址。列 metadata 使用 active/fill 双 slot，使 c2v 生产和 v2c 消费保持独立。
