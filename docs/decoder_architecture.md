...
- `decoder_top` 按 lane 显式实例化 RAM-I、RAM-S、RAM-T，并实例化 `2*L` 个 RAM-M bank 组成读写 pair。
- RAM-M 每个 numbered block 保存全局校验行地址空间中的压缩 c2v 状态和 1 bit epoch，深度为 `R`。顶层向 RAM-M、syndrome RAM 和验证侧传递 `row_idx_global`。
- RAM-I、RAM-S、RAM-T 的 lane 内 entry 深度统一为 `RAM_LANE_DEPTH`，表示首列元数据在各 lane 中的最大有效 entry 数。默认 BIKE-L1 形状参数下 `RAM_LANE_DEPTH=9`，`BIKE_TOY_PARAMS` 参数下 `RAM_LANE_DEPTH=3`。
- RAM-S 每个 lane 以 `S_PACK_W` bit word 保存 v2c sign bit。默认 BIKE-L1 配置使用 2 bit 打包，`BIKE_PARALLEL_L=2` 配置使用 8 bit 打包。顶层读写 shift buffer 在 entry bit 流和打包 word 之间转换；RAM-S 只提供 word 级读写。
...
RAM-I entry 保存 `{one_idx, row_idx_global}` 格式。解码期间，c2v 侧按 `h_block_idx` 和 entry 位置读取首列 entry，并在 `decoder_top` 中对 `base_row_idx_global + col_idx_local` 执行一次环绕修正，得到活跃列的行地址。v2c 元数据独立缓冲，使 c2v 侧可以领先一列。

### `$readmemh` 初始镜像路径

静态首列元数据由 `scripts/gen_qc_first_columns.py` 离线生成，输出 hex 文件到 `rtl/generated/`。该脚本读取 `rtl/bike_pkg.sv` 的尺寸参数，并使用 `scripts/qc_matrix_data.py` 中的首列支撑集。每个首列支撑集按 `one_idx` 派生的 lane 分入 RAM-I bank，entry 打包为 `{one_idx, row_idx_global}`。
...
| `one_idx` | `entry_pos * L + lane_idx` | RAM-S/RAM-T 边编号语义 |
...
- `merge_read/merge_write` 阶段由控制器独占 RAM-M 端口，正常 c2v 读请求在该阶段停发。
- RAM-S 每个 lane 一个 1R1W bank，c2v 读 sign，v2c 写 sign。`sign_bit_pack` 聚合 bit 并按 word flush，RAM-S 地址使用移位和低位选择生成。
