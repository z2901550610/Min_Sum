...
- `base_sign` 与 slot 0 的 `dev_pos` 打包为 8-bit 字段。
- slot 1 和 slot 2 分别使用 7-bit 字段。
- 每个字段按 RAMB36 的 `4K × 9` 原生几何划分深度段。
...
RAMB36 per bank  = 3 fields × 5 segments = 15
global K RAMB36  = 16 banks × 15 = 240
```

全局 K-sign RAM 占基线 RAMB36 总数的 `240 / 448`。tile 工作记录宽度为
...
| 资源 | 使用量 | 可用量 | 利用率 |
| --- | ---: | ---: | ---: |
| Slice LUT | 24,151 | 222,600 | 10.85% |
| LUT as Logic | 20,423 | 222,600 | 9.17% |
| LUT as Memory | 3,728 | 81,400 | 4.58% |
| Distributed RAM LUT | 3,520 | — | — |
| SRL LUT | 208 | — | — |
| Slice Register | 11,124 | 445,200 | 2.50% |
| Slice | 8,555 | 55,650 | 15.37% |
| Block RAM Tile | 489 | 715 | 68.39% |
| RAMB36E1 | 448 | 715 | 62.66% |
...
`ram_k_global` 分段 BRAM 的 enable 端口：

```text
Data Path Delay = 9.249 ns
logic           = 3.618 ns
route           = 5.631 ns
Logic Levels    = 8
```

该路径包含 correction 列地址计算、bank/segment 选择和 BRAM 使能译码。路径延迟以布线为主，
实现评估同时关注 WNS 和 route 比例。
