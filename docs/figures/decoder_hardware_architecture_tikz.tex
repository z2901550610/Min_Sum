...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[
      ...
      wire/.style={line width=0.55pt},
      ctrlwire/.style={-{Latex[length=1.2mm,width=0.9mm]}, line width=0.45pt, densely dashed},
      tap/.style={circle, fill=black, inner sep=0.95pt},
      label/.style={font=\tiny, inner sep=1pt, align=center}
    ]
    \node[proc, minimum width=25mm, minimum height=7mm] (sched) at (6.35,8.12)
      {tile scheduler};
    \node[mem, minimum width=20mm] (rami) at (1.15,7.22) {RAM I\\$H$ support};
    \node[proc, minimum width=21mm] (addr) at (3.85,7.22) {edge addr.\\generation};
    \node[proc, minimum width=26mm] (shiftwr) at (6.85,7.22)
      {$H$ matrix shifting\\RAM I writeback};
    ...
    \coordinate (ctrlbusL) at (0.70,8.74);
    \coordinate (ctrlbusR) at (15.85,8.74);

    \node[label, anchor=south] at (10.70,8.98)
      {public control: support set, tile, row, edge, buffer};
    \draw[ctrlwire] (ctrlbusL) -- (ctrlbusR);
    \draw[ctrlwire] (6.35,8.74) -- (sched.north);
    \draw[ctrlwire] (sched.south west) -- ++(-0.12,-0.18) -| (rami.north);
    \draw[ctrlwire] (sched.south) -- ++(0,-0.18) -| (addr.north);
    \draw[ctrlwire] (sched.south east) -- ++(0.12,-0.18) -| (shiftwr.north);
    \draw[ctrlwire] (sched.east) -- ++(7.80,0) |- (wbusTop);
    \node[tap] at (6.35,8.74) {};
    \draw[flow] (rami.east) -- (addr.west);
    \draw[flow] (addr.east) -- node[label, above] {addr} (shiftwr.west);
