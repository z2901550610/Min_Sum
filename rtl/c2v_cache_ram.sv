...
(
    ...
    input  logic        [  Q_SEQ_W-1:0] i_c2v_q_seq,
  ...
  generate
    for (genvar buf_idx = 0; buf_idx < 2; buf_idx++) begin : g_buf
      for (genvar lane_idx = 0; lane_idx < L; lane_idx++) begin : g_lane
        (* ram_style = "distributed" *) logic signed [   ACC_W-1:0] mem[0:T_DEPTH-1];
        logic                       bank_re;
        ...
        always_comb begin
          ...
          bank_we = i_c2v_valid[lane_idx] && (int'(i_fill_buf) == buf_idx);
          bank_waddr = t_addr(i_c2v_one_idx, i_c2v_q_seq);
        end

        assign bank_rdata[buf_idx][lane_idx] = bank_re ? mem[bank_raddr] : '0;

        always_ff @(posedge i_clk) begin
          if (bank_we) begin
            mem[bank_waddr] <= i_c2v_wdata[lane_idx];
          end
        end
      end
    end
