...
`endif
  ...
  logic        [        ROW_IDX_W-1:0] unused_ksign_check_row_idx[0:L-1];
  ...
  logic                                ksign_overlap_phase;
  logic        [        H_BLOCK_W-1:0] ksign_overlap_h_block_idx;
  ...
  logic        [        ROW_IDX_W-1:0] ksign_h_base_row_idx;
  ...
  always_comb begin
    ...
    for (int lane_idx = 0; lane_idx < L; lane_idx++) begin
      ...
      if (K_SIGN_ENABLE && !c2v_iter_zero_c) begin
        c2v_cnu_b_comp[lane_idx][COMP_C2V_SIGN_XOR_BIT] =
            c2v_comp_c[lane_idx][COMP_C2V_SIGN_XOR_BIT] ^ ksign_delta_c[lane_idx];
      end
  ...
  ram_i u_ram_i (
      ...
      .i_c2v_diag_idx_local(active_diag_idx_local),
      .i_v2c_h_block_idx(v2c_h_block_idx),
      .i_v2c_diag_idx_local(active_diag_idx_local),
      .i_corr_h_block_idx(ksign_overlap_h_block_idx),
      .i_corr_diag_idx_local(ksign_overlap_diag_idx_local),
      .i_cfg_r(cfg_r),
      .i_cfg_w(cfg_w),
      .o_c2v_base_row_idx(c2v_h_base_row_idx),
      .o_v2c_base_row_idx(v2c_h_base_row_idx),
      .o_corr_base_row_idx(ksign_h_base_row_idx),
      .o_loaded(o_h_loaded),
      .o_error(o_h_error)
  );
  ...
  tile_scheduler u_tile_scheduler (
      ...
      .i_ksign_corr_done(ksign_overlap_done),
      .i_cfg_w(cfg_w),
  ...
  assign ksign_overlap_start = K_SIGN_ENABLE && v2c_phase_active &&
      (v2c_tile_linear == '0) &&
      (active_diag_idx_local == DIAG_IDX_W'(cfg_w - CFG_W_W'(1))) &&
      (active_lane_group_idx == LANE_GROUP_IDX_W'(Q_TILE - 1));

  k_sign_overlap_scheduler u_k_sign_overlap_scheduler (
      .i_clk(i_clk),
      .i_rst_n(rst_n_sync),
      .i_start(ksign_overlap_start),
      .i_cfg_w(cfg_w),
      .i_cfg_tile_count(cfg_tile_count),
      .o_phase_valid(ksign_overlap_phase),
      .o_h_block_idx(ksign_overlap_h_block_idx),
  ...
  edge_addr_gen u_ksign_overlap_addr_gen (
      .i_clk(i_clk),
      .i_rst_n(rst_n_sync),
      .i_phase_valid(ksign_scan_phase_e),
      .i_h_block_idx(ksign_scan_h_block_idx_e),
      .i_tile_idx(ksign_scan_tile_idx_e),
      .i_lane_group_idx(ksign_scan_lane_group_idx_e),
      .i_base_row_idx(ksign_scan_h_base_row_idx_e),
      .i_diag_idx_local(ksign_scan_diag_idx_local_e),
      .i_cfg_r(cfg_r),
      .i_cfg_w(cfg_w),
      .o_valid(ksign_scan_valid),
      .o_check_row_idx(unused_ksign_check_row_idx),
      .o_col_idx(ksign_scan_col_idx),
      .o_diag_idx_global_base(unused_ksign_diag_idx_global_base),
      .o_diag_idx_global(unused_ksign_diag_idx_global),
      .o_row_bank(unused_ksign_row_bank),
      .o_row_addr(ksign_scan_row_addr),
      .o_tile_offset(ksign_scan_tile_offset)
  );
  ...
  generate
    if (K_SIGN_ENABLE) begin : g_sign_k
      ...
      ram_sign_delta u_ram_sign_delta (
          ...
          .i_clear_pair_sel(comp_write_pair_sel),
          .i_clear_row_addr(comp_clear_addr),
          .i_read_pair_sel(comp_read_pair_sel_r),
          .i_read_valid(c2v_valid_r),
          .i_read_row_addr(c2v_row_addr_r),
          .o_read_delta(ksign_delta_read_mem),
          .i_flip_pair_sel(comp_write_pair_sel),
          .i_flip_valid(ksign_delta_flip_valid),
          .i_flip_row_addr(ksign_scan_row_addr_r)
      );
  ...
  always_ff @(posedge i_clk or negedge rst_n_sync) begin
    if (!rst_n_sync) begin
      ...
      for (int lane_idx = 0; lane_idx < L; lane_idx++) begin
        ...
        ksign_delta_s[lane_idx] <= 1'b0;
        ksign_delta_c[lane_idx] <= 1'b0;
      ...
      ksign_scan_phase_h <= ksign_overlap_phase;
      ksign_scan_h_block_idx_h <= ksign_overlap_h_block_idx;
      ksign_scan_tile_idx_h <= ksign_overlap_tile_idx;
      ksign_scan_diag_idx_local_h <= ksign_overlap_diag_idx_local;
      ksign_scan_lane_group_idx_h <= ksign_overlap_lane_group_idx;
      ...
      for (int lane_idx = 0; lane_idx < L; lane_idx++) begin
        ...
        ksign_delta_s[lane_idx] <= ksign_delta_read_mem[lane_idx];
        ...
        ksign_delta_c[lane_idx] <= ksign_delta_s[lane_idx];
