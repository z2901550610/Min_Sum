...
#(
    ...
    input  logic [((P_T_VALS[3] > 1) ? $clog2(P_T_VALS[3]) : 1)-1:0] i_sampler_index_position,
    input  logic [(((3 * P_R_VALS[3]) > 1) ? $clog2(3 * P_R_VALS[3]) : 1)-1:0] i_sampler_index,
    ...
    output logic [((P_T_VALS[3] > 1) ? $clog2(P_T_VALS[3]) : 1)-1:0] o_h4_store_index_position,
    output logic [(((3 * P_R_VALS[3]) > 1) ? $clog2(3 * P_R_VALS[3]) : 1)-1:0] o_h4_store_index,
    input  logic i_h4_store_index_ready,
    output logic o_h4_store_support_re,
    output logic [((P_T_VALS[3] > 1) ? $clog2(P_T_VALS[3]) : 1)-1:0] o_h4_store_support_raddr,
    input  logic [(((3 * P_R_VALS[3]) > 1) ? $clog2(
3 * P_R_VALS[3]
    ) : 1)-1:0] i_h4_store_support_rdata,
    output logic o_h4_store_error_re,
    output logic [(((3 * (((P_R_VALS[3] + 511) / 512) * 64)) > 1) ? $clog2(
3 * (((P_R_VALS[3] + 511) / 512) * 64)
