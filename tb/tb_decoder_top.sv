...
module tb_decoder_top;
  ...
  logic   [    GROUP_COUNT_W-1:0] prev_ram_i_shift_count_wdata[0:B-1];
  ...
  task automatic apply_reset;
    begin
      ...
      for (int group_idx = 0; group_idx < B; group_idx++) begin
      ...
      if (prev_ram_i_shift_count_we) begin
        for (int group_idx = 0; group_idx < B; group_idx++) begin
          if (dut.ram_i_debug_count[prev_ram_i_shift_h_block_idx][group_idx] !=
            ...
            $fatal(1, "RAM-I committed count mismatch group %0d: got %0d exp %0d", group_idx,
  ...
  initial begin
    ...
    #1;
    if (dut.ram_i_debug_count[0][0] != GROUP_COUNT_W'(2))
      $fatal(1, "RAM I shifted group_idx0 count mismatch for column 1");
    if (dut.ram_i_debug_count[0][1] != GROUP_COUNT_W'(1))
      $fatal(1, "RAM I shifted group_idx1 count mismatch for column 1");
    if (!(dut.c2v_group_valid[0] && dut.c2v_group_valid[1]))
      $fatal(1, "shifted column 1 should expose two active group_idxs");
    if (dut.c2v_one_idx[0] != ONE_IDX_W'(1) || dut.c2v_row_idx_group[0] != ROW_GROUP_W'(1))
      $fatal(1, "shifted group_idx0 entry mismatch for column 1");
    if (dut.c2v_one_idx[1] != ONE_IDX_W'(0) || dut.c2v_row_idx_group[1] != ROW_GROUP_W'(0))
      $fatal(1, "shifted group_idx1 entry mismatch for column 1");
    ...
    for (idx = 0; idx < L; idx++) begin
      if (dut.vnu_accum_valid[idx]) begin
        ...
        if (int'(dut.c2v_tc[idx]) != c2v_signmag_to_tc(
                CASE1_FIRST_C2V_SIGN[flat_idx], CASE1_FIRST_C2V_MAG[flat_idx]
            )) begin
          $fatal(1, "CASE1 RAM-T slot c2v tc[%0d] mismatch: got %0d exp %0d", flat_idx,
                 int'(dut.c2v_tc[idx]), c2v_signmag_to_tc(CASE1_FIRST_C2V_SIGN[flat_idx],
                                                          CASE1_FIRST_C2V_MAG[flat_idx]));
        end
      end
    end
    ...
    wait (iter_count == 1);
    #1;
    if (dut.m_read_pair === dut.m_write_pair)
