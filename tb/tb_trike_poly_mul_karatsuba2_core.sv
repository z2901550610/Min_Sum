...
module tb_trike_poly_mul_karatsuba2_core;
  localparam int R_BITS = 257;
  ...
  localparam int EXPECTED_CYCLES =
      (9 * QUARTER_WORDS * QUARTER_WORDS) + (152 * QUARTER_WORDS) + (6 * WORDS) - 9;
  ...
  trike_poly_mul_karatsuba2_core #(
      .R_BITS              (R_BITS),
      .WORD_W              (WORD_W),
  ...
  initial begin
    ...
    for (int bit_idx = 0; bit_idx < R_BITS; bit_idx++) begin
      case_a[1][bit_idx] = (((bit_idx * 17) + 3) % 11) < 5;
      case_b[1][bit_idx] = (((bit_idx * 29) + 1) % 13) < 6;
    end
    case_a[2] = {R_BITS{1'b1}};
    case_b[2] = '0;
    case_b[2][0] = 1'b1;
    case_b[2][64] = 1'b1;
    case_b[2][128] = 1'b1;
    ...
    for (int test_idx = 0; test_idx < 3; test_idx++) begin
      if (cycles[test_idx] != EXPECTED_CYCLES) begin
        $fatal(1, "case %0d cycles=%0d expected=%0d", test_idx, cycles[test_idx], EXPECTED_CYCLES);
      end
    end

    $display("tb_trike_poly_mul_karatsuba2_core PASS cycles=%0d", cycles[0]);
    $finish;
  end

  initial begin
    repeat (3000) @(posedge clk);
    $fatal(1, "tb_trike_poly_mul_karatsuba2_core timeout");
  end
endmodule
