...
module tb_trike_poly_mul_karatsuba2_reference;
  `include "generated/trike_poly_mul_reference_case.svh"

  localparam int QUARTER_WORDS = (REF_WORDS + 3) / 4;
  localparam int REF_DENSE_CYCLES =
      (9 * QUARTER_WORDS * QUARTER_WORDS) + (152 * QUARTER_WORDS) +
      (6 * REF_WORDS) - 9;
  ...
  initial begin
    ...
    $display("tb_trike_poly_mul_karatsuba2_reference PASS dense=%0d", dense_cycles);
    $finish;
  end

  initial begin
    repeat (REF_DENSE_CYCLES + 1000) @(posedge clk);
    $fatal(1, "tb_trike_poly_mul_karatsuba2_reference timeout");
  end
endmodule
